...
int main() {
	...
	static const float interleaved2nd[] = {
		//     X      Y     Z       R     G    B
			 1.0f,  2.0f, 0.0f,  0.8f, 0.8f, 0.4f,
			 1.0f,  0.1f, 0.0f,  1.0f, 1.0f, 0.1f,
			 -0.7f,  0.3f, 0.0f,  0.5f, 0.0f, 0.5f
	};

	VertexBuffer::sptr interleaved_vbo2nd = VertexBuffer::Create();
	interleaved_vbo2nd->LoadData(interleaved2nd, 6 * 4);

	static const uint16_t indices2nd[] = {
		0, 1, 2,
		1, 3, 2
	};
	IndexBuffer::sptr interleaved_ibo2nd = IndexBuffer::Create();
	interleaved_ibo2nd->LoadData(indices2nd, 3 * 2);

	size_t stride2nd = sizeof(float) * 6;
	VertexArrayObject::sptr vao2nd = VertexArrayObject::Create();
	vao2nd->AddVertexBuffer(interleaved_vbo2nd, {
		BufferAttribute(0, 3, GL_FLOAT, false, stride2nd, 0),
		BufferAttribute(1, 3, GL_FLOAT, false, stride2nd, sizeof(float) * 3),
		});
	vao2nd->SetIndexBuffer(interleaved_ibo2nd);
	...
	glm::mat4 transform2nd = glm::mat4(1.0f);
	...
	while (!glfwWindowShouldClose(window)) {
		...
		transform2nd = glm::rotate(glm::mat4(1.0f), static_cast<float>(thisFrame), glm::vec3(1, 1, 0)) + glm::translate(glm::mat4(1.0f), glm::vec3(0, glm::sin(static_cast<float>(thisFrame)), 0.0f));
		...
		shader->SetUniformMatrix("u_ModelViewProjection", camera->GetViewProjection() * transform2nd);

		vao2nd->Bind();
		glDrawElements(GL_TRIANGLES, interleaved_ibo2nd->GetElementCount(), interleaved_ibo2nd->GetElementType(), nullptr);
		vao2nd->UnBind();
